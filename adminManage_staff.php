      ...
      if ($checkTokenResult) {
          ...
          if ($token === $dbToken) {
              ...
              if (isset($_POST['save'])) {
                ...
                //update user table
                $sql = "UPDATE user SET user_phone = '$phone', user_name = '$name', user_age='$age', user_pass = '$password', role_id = '$role', update_at = NOW() WHERE user_id = '$staffID'";
                $result = mysqli_query($conn, $sql);
                
                if ($result) {

                  echo "<script>alert('Staff Information has been updated!');</script>";
                  ...
                  if (mysqli_num_rows($checkEmailResult) > 0) {
                      ...
                      $sql = "UPDATE user SET user_email = '$email' WHERE user_id = '$staffID'";
                      $result = mysqli_query($conn, $sql);
            ...
            elseif (isset($_POST['back'])) {
              echo "<script>window.location.href = 'adminStaffList.php';</script>";
            } 
          } else {
...
<section class="pt-7 px-7 md:px-20 w-full">
    <h2 class="text-3xl font-bold mb-4 text-white">Staff Details</h2>
...
<div>
    <div class="container rounded bg-white mt-5 p-10 w-full">
                ...
                <form action="" method="POST">
                    ...
                    <?php
                      $sql = "SELECT * FROM user JOIN role  ON user.role_id = role.role_id WHERE user_id = '$staffID'";
                    ...
                    <div class="flex flex-col md:flex-row gap-0 md:gap-4">
                      <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          ...
                          <input required type="text" name="name" class="form-control p-1 w-full" placeholder="Name" value="<?php echo $row['user_name'] ?>">
                        </div>
                      </div>
                      <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          <label class="bg-white absolute left-3 top-0 text-xs px-1" for="">Email</label>
                          <input required type="text" name="email" class="form-control p-1 w-full" placeholder="Email" value="<?php echo $row['user_email'] ?>">
                        </div>
                      </div>
                    </div>
                    <div class="flex flex-col md:flex-row gap-0 md:gap-4">
                      <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          ...
                          <input type="text" name="age" class="form-control p-1 w-full" placeholder="Age" value="<?php echo $row['user_age'] ?>">
                        </div>
                      </div>
                      <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          <label class="bg-white absolute left-3 top-0 text-xs px-1" for="">Phone Number</label>
                          <input required type="text" name="phone" class="form-control p-1 w-full" placeholder="Phone Number" value="<?php echo $row['user_phone'] ?>">
                        </div>
                      </div>
                    </div>

                    <div class="flex flex-col md:flex-row gap-0 md:gap-4">
                    <div class="mt-2 relative w-full" style=" padding-top:5px">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          <label class="bg-white absolute left-3 top-0 text-xs px-1" style="padding-top:2px" for="">Password</label>
                          <input type="password" name="password" class="form-control p-1 w-full" placeholder="Password" value="<?php echo $row['user_pass'] ?>">
                        </div>
                    </div>
                    <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          <label class="bg-white absolute left-3 top-0 text-xs px-1" for="">Role</label>
                          <select required name="role" class="form-control p-1 w-full">
                            <?php
                              // Query to fetch roles from the role table
                              $roleQuery = "SELECT * FROM role";
                              $roleResult = mysqli_query($conn, $roleQuery);

                              if (mysqli_num_rows($roleResult) > 0) {
                                while ($roleRow = mysqli_fetch_assoc($roleResult)) {
                                  // Check if the current role is the selected role for the user
                                  $selected = ($roleRow['role_id'] == $row['role_id']) ? 'selected' : '';
                                  echo '<option value="' . $roleRow['role_id'] . '" ' . $selected . '>' . $roleRow['role_title'] . '</option>';
                                }
                              }
                            ?>
                          </select>
                    ...
                    <!-- <input type="text" name="token" class="form-control p-1 w-full" placeholder="token" value="<?php echo $row['token'] ?>"> -->
                    <br><br>

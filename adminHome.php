   ...
   if($_SESSION['user_id']){
      ...
      if ($checkTokenResult) {
          ...
          if ($token === $dbToken) {
              ...
              $countStaff = mysqli_num_rows($result2);
              
              //count books and calculate total sales
              $sql2 = "SELECT * FROM booking";
              $result3 = mysqli_query($conn, $sql2);
              $countBooking = mysqli_num_rows($result3);
              ...
              if($result3){
                ...
                //count completed booking
                $sql3 = "SELECT COUNT(*) AS paid_count FROM booking WHERE booking_history = 'Paid'";
                $result = mysqli_query($conn, $sql3);
                
                if ($result) {
                    $row = mysqli_fetch_assoc($result);
                    $countCompletedBooking = $row['paid_count'];
                }

                $sql4 = "SELECT COUNT(*) AS count FROM booking WHERE booking_history = 'Pending' OR booking_history = 'In Process'";
                $result3 = mysqli_query($conn, $sql4);
                
                if ($result3) {
                    $row = mysqli_fetch_assoc($result3);
                    $countPendingBooking = $row['count'];
...
<section class="pt-7 px-7 md:px-20 w-full overflow-y-auto">
    ...
    <div class="bg-white rounded-lg table-auto p-2 text-left flex flex-row w-full gap-10 text-white text-center p-10">
      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold"><?php echo $countCustomer ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Customers</h1>
      </div>

      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold"><?php echo $countStaff ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Staffs</h1>
      </div>

      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold">RM <?php echo $totalSales ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Overall Sales</h1>
      </div>
    </div>
    <div class="bg-white rounded-lg table-auto p-2 text-left flex flex-row w-full gap-10 text-white text-center p-10">
      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold"><?php echo $countBooking ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Bookings</h1>
      </div>

      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold"><?php echo $countCompletedBooking ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Completed Bookings</h1>
      </div>

      <div class="rounded-lg w-1/3 bg-dtarblue p-10">
      <h1 class="text-4xl font-bold"><?php echo $countPendingBooking ?></h1>
      <hr class="my-2">
      <h1 class="text-2xl italic">Pending Bookings</h1>
      </div>
    </div>

      ...
      if ($checkTokenResult) {
          ...
          if ($token === $dbToken) {
              ...
              if (isset($_GET['delete'])) {
                ...
                if($result){
                  echo '<script>alert("The selected staff account has been deleted!");</script>';
...
<div class="bg-white rounded-lg table-auto p-2 text-left">
    <table class="w-full">
        ...
        <?php 
            ...
            $sql = "SELECT * FROM user JOIN role  ON user.role_id = role.role_id WHERE user_id != '$userID'";
            $result = mysqli_query($conn, $sql);

  ...
  <section class="pt-7 px-7 md:px-20 w-full overflow-y-auto">
    ...
    <form method="POST">
      <div class="flex flex-row gap-5 my-4">
        <h2 class="text-3xl font-bold text-dtarblue ml-3 md:ml-10 my-auto "><?php echo "$month $year"; ?> </h2>
        <div class="mt-4 relative  w-72">
          <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
            ...
            <input disabled style="background-color: #c3c3c3;" type="text" class="form-control px-1 py-2  w-full font-semibold"  value="<?php echo $_SESSION['hall_name']; ?>">      
          </div>
        </div>

        <div class="mt-4 relative w-">
          <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
            <label class="bg-white absolute left-3 top-0 text-xs px-1" for="">Court</label>
            <select class="form-control px-1 py-2 w-full font-semibold" name="court_id">
              <?php
              $hall_id = $_SESSION['hall_id'];

              $courtQuery = "SELECT * FROM court WHERE hall_id = '$hall_id'";
              $courtResult = mysqli_query($conn, $courtQuery);

              if ($courtResult && mysqli_num_rows($courtResult) > 0) {
                while ($courtRow = mysqli_fetch_assoc($courtResult)) { 
                    $selected = ($courtRow['court_id'] == $court_id) ? 'selected' : '';
                    echo '<option value="' . $courtRow['court_id'] . '"' . $selected . '>' . $courtRow['court_num'] . '</option>';
                }
            }
              ?>
        </select>
        ...
        <button class="p-2 text-black rounded bg-dtargreen text-2xl h-full mt-auto" name='submit'><span aria-hidden="true">&rarr;</span></button>
          ...
          <div class="ml-auto my-auto">
            ...
            ?>
            <a class="bg-dtargreen text-black  rounded px-3 py-1 mr-3" href="<?php echo $_SERVER['PHP_SELF'] . '?week=' . date('W') . '&year=' . date('o'); ?>">Current Week</a>
            <a class="bg-dtargreen text-black  rounded px-3 py-1 mr-3" href="<?php echo $_SERVER['PHP_SELF'] . '?week=' . ($week + 1) . '&year=' . $year; ?>">Next Week</a>

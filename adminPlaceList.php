 ...
 if($_SESSION['user_id']){
      ...
      if ($checkTokenResult) {
          ...
          if ($token === $dbToken) {
              ...
              // Check if the id and table parameters are set in the URL
              if(isset($_GET['id']) && isset($_GET['table'])) {
                  ...
                  if (isset($_GET['table'])) {
                    ...
                    if ($table === 'court') {
                        // Retrieve the current court availability
                        $query = "SELECT court_availability FROM court WHERE court_id = '$id'";
                        $result = mysqli_query($conn, $query);
                        $row = mysqli_fetch_assoc($result);
                        $currentAvailability = $row['court_availability'];
                
                        // Determine the new availability status
                        $newAvailability = ($currentAvailability === 'available') ? 'not available' : 'available';
                
                        // Perform the update query for the court table
                        $updateSql = "UPDATE court SET court_availability = '$newAvailability' WHERE court_id = '$id'";
                        $updateResult = mysqli_query($conn, $updateSql);
                
                        if ($updateResult) {
                            echo '<script>alert("The selected court availability has been updated!");</script>';
                            echo "<script>window.location.href = 'adminPlaceList.php';</script>";
                        } else {
                            echo "Error updating court availability: " . mysqli_error($conn);
                        }
                    } elseif ($table === 'hall') {
                        // Retrieve the current hall availability
                        $query = "SELECT hall_availability FROM hall WHERE hall_id = '$id'";
                        $result = mysqli_query($conn, $query);
                        $row = mysqli_fetch_assoc($result);
                        $currentAvailability = $row['hall_availability'];
                
                        // Determine the new availability status
                        $newAvailability = ($currentAvailability === 'available') ? 'not available' : 'available';
                
                        // Perform the update query for the hall table
                        $updateSql = "UPDATE hall SET hall_availability = '$newAvailability' WHERE hall_id = '$id'";
                        $updateResult = mysqli_query($conn, $updateSql);
                
                        if ($updateResult) {
                            echo '<script>alert("The selected hall availability has been updated!");</script>';
                            echo "<script>window.location.href = 'adminPlaceList.php';</script>";
                        } else {
                            echo "Error updating hall availability: " . mysqli_error($conn);
                        }
                    }
                  }}                
...
<div class="bg-white rounded-lg table-auto p-2 text-left">
    <table class="w-full">
        <tr class="bg-gray-100 " style="text-align: center;">
            <th class="p-2">No</th>
            <th class="p-2">Hall ID</th>
            <th class="p-2">Hall Name</th>
...
<h2 class="text-3xl font-bold mb-4 mt-20 text-white">Manage Customer</h2>
<div class="bg-white rounded-lg table-auto p-2 text-left">
    <table class="w-full">
        <tr class="bg-gray-100 " style="text-align: center;">
            <th class="p-2">No</th>
        ...
        <?php 
            ...
            if (mysqli_num_rows($result) > 0) {
              ...
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                <tr style="text-align: center;">
                    <th class="p-2">' . $count . '</th>
                    <th class="p-2">' . $row['court_id'] . '</th>
                    <th class="p-2">' . $row['court_num'] . '</th>';
                    
                    if($row['court_availability'] == 'available'){
                      echo '<th class="p-2">' . $row['court_availability'] . '</th>';

                    }
                    else{
                      echo '<th class="p-2" style="color:red; font-size:16px;">' . $row['court_availability'] . '</th>';

                    }                    
                    echo' <th class="p-2">' . $row['hall_id'] . '</th>
                    <th>

                    <a onClick="javascript: return confirm(\'Are you sure you want to update this court availability?\');" href="'.$_SERVER['PHP_SELF'].'?id='.$row['court_id'].'&table=court"> <button class="text-white p-2 rounded mr-1 my-1" style="background-color: green;"><svg xmlns="http://www.w3.org/2000/svg" fill="white" height="1em" viewBox="0 0 576 512"><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M402.6 83.2l90.2 90.2c3.8 3.8 3.8 10 0 13.8L274.4 405.6l-92.8 10.3c-12.4 1.4-22.9-9.1-21.5-21.5l10.3-92.8L388.8 83.2c3.8-3.8 10-3.8 13.8 0zm162-22.9l-48.8-48.8c-15.2-15.2-39.9-15.2-55.2 0l-35.4 35.4c-3.8 3.8-3.8 10 0 13.8l90.2 90.2c3.8 3.8 10 3.8 13.8 0l35.4-35.4c15.2-15.3 15.2-40 0-55.2zM384 346.2V448H64V128h229.8c3.2 0 6.2-1.3 8.5-3.5l40-40c7.6-7.6 2.2-20.5-8.5-20.5H48C21.5 64 0 85.5 0 112v352c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48V306.2c0-10.7-12.9-16-20.5-8.5l-40 40c-2.2 2.3-3.5 5.3-3.5 8.5z"/></svg></button></a>
                    </th>
                </tr>';

          ...
          if ($token === $dbToken) {
              ...
              if (isset($_POST['save'])) {
                ...
                $sql = "UPDATE customer SET customer_ic='$ic', customer_phone='$phone', customer_address='$address',
                        customer_name='$name', customer_age='$age', customer_pass = '$password', update_at = NOW() WHERE customer_id = '$customerID'";
                ...
                if ($result) {
                  ...
                  //update customer table
                  $sql = "UPDATE online_payment SET after_balance = after_balance + '$reload',  update_at = NOW() WHERE customer_id = '$customerID'";
                  $result = mysqli_query($conn, $sql);

                  if ($result) {
                    echo "<script>alert('Customer e-Wallet has been updated!');</script>";
...
<section class="pt-7 px-7 md:px-20 w-full">
    <h2 class="text-3xl font-bold mb-4 text-white">Customer Details</h2>
...
<div>
    <div class="container rounded bg-white mt-5 p-10 w-full">
                ...
                <form action="" method="POST">
                    ...
                    <div class="mt-2 relative w-full" style="width:49.5%; padding-top:5px">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2">
                          <label class="bg-white absolute left-3 top-0 text-xs px-1" style="padding-top:2px" for="">Password</label>
                          <input type="password" name="password" class="form-control p-1 w-full" placeholder="Password" value="<?php echo $row['customer_pass'] ?>">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-0 md:gap-4">
                      ...
                      <div class="mt-2 relative w-full">
                        <div class="border-2 border-gray-400 p-1 rounded-lg mt-2" >
                          <label class="bg-white absolute left-3 top-0 text-xs px-1"for="">Reload Amount: </label>
                          <input type="number" name="reload" class="form-control p-1 w-full" placeholder="Topup amount from customer" value="">
                        </div>
                      </div>
                    </div><br>
